...
interface ModelOccurrence {
  ...
  locations: { uri: vscode.Uri; line: number; col: number; preview: string }[]
}

export async function runWorkspaceCostScan(context: vscode.ExtensionContext): Promise<void> {
  const panel = vscode.window.createWebviewPanel(
    'orCostScanner',
    'OR Intelligence: Workspace Cost Scanner',
  ...
  const files = await vscode.workspace.findFiles(
    '**/*.{ts,tsx,js,jsx,py,go,rb,java,cs,php,rs,swift,kt,yaml,yml,json,toml,env,txt,md}',
    '**/node_modules/**',
  )

  const occurrenceMap = new Map<string, ModelOccurrence>()
  ...
  const occurrences = [...occurrenceMap.values()].sort(
...
function estimateMonthly(model: Model | undefined, monthlyVol: number, avgOut: number): number {
  if (!model) return 0
  const avgIn = 200
...
function estimateMonthlySavings(current: Model, alt: Model, monthlyVol: number, avgOut: number): number {
  return Math.max(0, estimateMonthly(current, monthlyVol, avgOut) - estimateMonthly(alt, monthlyVol, avgOut))
}

function buildCheaperMap(occurrences: ModelOccurrence[], allModels: Model[]): Map<string, Model> {
