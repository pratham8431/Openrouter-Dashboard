...
if (!existsSync(CONFIG_PATH)) {
  console.error('❌  cron/config.json not found. Export it from the dashboard.')
  process.exit(1)
}
...
if (!email || !rules.length) {
  console.log('ℹ  No email or watch rules in config — nothing to do.')
  process.exit(0)
}
...
if (!res.ok) { console.error(`❌  OpenRouter API ${res.status}`); process.exit(1) }
const { data: freshModels } = await res.json()
...
if (existsSync(SNAPSHOT_PATH)) {
  try { snapshot = JSON.parse(readFileSync(SNAPSHOT_PATH, 'utf8')) } catch { /* treat as empty */ }
}

// ── Diff — check each rule ─────────────────────────────────────────────────────

const triggered = []   // { rule, change }
...
for (const rule of rules) {
  ...
  if (prev.price !== fresh.pricing.prompt) {
    ...
    // Only trigger if no threshold set, OR threshold is crossed
    if (!priceThreshold || currentPricePerM > priceThreshold) {
      ...
      console.log(`ℹ  ${fresh.name} price changed but $${currentPricePerM.toFixed(2)}/M is still under your $${priceThreshold}/M threshold.`)
    }
  }
}
...
if (!triggered.length) {
  console.log('✅  No triggered rules detected.')
  process.exit(0)
}

console.log(`⚠  ${triggered.length} rule(s) triggered — getting fallback recommendations...`)

// ── Get Claude fallback for each triggered rule ─────────────────────────────────

async function getFallback(rule, change) {
...
const results = await Promise.all(
  triggered.map(async t => ({ ...t, fallback: await getFallback(t.rule, t.change) }))
)

// ── Build email ─────────────────────────────────────────────────────────────────

if (!gmailUser || !gmailAppPassword) {
  console.log('📋  Triggered changes (no email credentials set):')
  results.forEach(({ change, fallback }) => {
    console.log(`  ${change.type.toUpperCase()}: ${change.name} ${change.detail ?? ''}`)
    if (fallback) console.log(`  → Switch to: ${fallback.modelName} (${fallback.modelId})`)
  })
  process.exit(0)
}
...
<body style="background:#0A0A0F;color:#E8E8F0;font-family:system-ui,sans-serif;padding:32px;max-width:640px">
  ...
  <p style="font-family:monospace;font-size:10px;color:#4A4A60;margin-top:24px;border-top:1px solid rgba(255,255,255,0.07);padding-top:16px">
    Sent by OpenRouter Model Intelligence ·
    <a href="https://openrouter.ai/models" style="color:#6366F1">View models</a>
  </p>
</body>
</html>`
